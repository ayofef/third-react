...
const GET_BLOG_DATA = gql`
  query($where: String!) {
    blogsConnection(
      ...
      stage: PUBLISHED
      where: { postCategory_contains: $where }
    ) {
      edges {
        node {
          id
...
function Blogs(props) {
  ...
  const [sort, setSort] = useState({
    value: props.match.params.whose,
    label: props.match.params.whose.toUpperCase(),
  });
  ...
  const { loading, error, data } = useQuery(GET_BLOG_DATA, {
    variables: { where: whose === "all" ? "" : whose },
  });
  ...
  const totalPages = Math.ceil(
    data.blogsConnection.edges.length / BlogListPerPage
  );
  const BlogList = data.blogsConnection.edges.slice(
    indexOfFirstBlogList,
    indexOfLastBlogList
  );
          ...
          <div className="latestNewsAndResults">
            <div className="latestNewsContainer">
              <div className="blog-post__nav">
                <h1 className="u-heading blog-posts__whose-heading">
                  {headerText} <span> Latest News</span>
                </h1>
                <div className="latest-news__sort-search">
                  <div className="latest-news__search">
                    <input
                      className="latest-news__search--input"
                      placeholder="Search.."
                      list="sugesst"
                      name="search"
                      type="text"
                      ...
                      onKeyPress={(event) =>
                        event.key === "Enter" && searched.length > 1
                          ? history.push(
                              `/latest-news/${whose}/search?query=${searched}`
                            )
                          : null
                      }
                    />
                    ...
                    <Link
                      className={
                        searched.length < 1
                          ? "latest-news__search--disabled"
                          : "latest-news__search--link"
                      }
                      to={`/latest-news/${whose}/search?query=${searched}`}
                      onClick={(event) =>
                        searched.length < 1 ? event.preventDefault() : true
                      }
                    >
                      <span>
              ...
              <div className="blog-posts__container">
                {BlogList.map(({ node }) => (
                  <BlogCard style={props} node={node} key={node.id} />
                ))}
              </div>
              <div style={{ marginBottom: "3rem" }}>
                {data.blogsConnection.edges.length > BlogListPerPage ? (
                  <Pagination
                    prev={() => prevPage(totalPages)}
