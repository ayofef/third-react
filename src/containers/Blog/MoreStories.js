        ...
        <aside className="more-stories">
            ...
            {data.blogs.map(el => <div className="blog-posts__card" key={el.id}>
                ...
                <div className="blog-posts__card--text">
                    ...
                    <p className="blog-posts__card--text-date">{DateFormatter(el.createdAt)}</p>
                    <Link to={`/latest-news/${whose}/${el.postHeading.split(" ").join("_")}`} className="blog-posts__card--link">Continue Reading<span><svg className="arrow-button-icon">
