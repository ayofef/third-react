...
function SectionBlog(props) {
  return (
    <section
      className={["latest-news", `${props.identifier + "-latest-news"}`].join(
        " "
      )}
      id={props.identifier + "-latest-news"}
    >
      <div className="container">
        <h2
          className={["u-heading", `${props.identifier + "-heading"}`].join(
            " "
          )}
        >
          LATEST NEWS
        ...
        <Link to={props.path} className="btn latest-news__link">
          More News
          <span>
            <svg className="arrow-button-icon" aria-hidden="true">
